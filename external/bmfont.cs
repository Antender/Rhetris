...
namespace BmFont
{
	...
	public FontRenderer(string fontFileName, string textureFileName)
	{
		var fontFilePath = Path.Combine(Content.RootDirectory, fontfileName);
		using(var stream = TitleContainer.OpenStream(fontFilePath))
		{
			var fontFile = FontLoader.Load(stream);
			var fontTexture = Content.Load<Texture2D>(textureFileName);
			stream.Close();
			FontRenderer(fontFile,fontTexture);
		}
	}
